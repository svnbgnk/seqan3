...
inline void search_algo_uni(index_t const & index, query_t & query, search_param const error_left,
                            delegate_t && delegate, delegate_itv_t && delegate_itv)
{
    search_trivial<abort_on_hit>(index, query, error_left, delegate, delegate_itv);
}
 ...
 */
template <bool abort_on_hit, typename index_t, typename query_t, typename delegate_t, typename delegate_itv_t>
inline void search_algo(index_t const & index, query_t & query, search_param const error_left, delegate_t && delegate, delegate_itv_t && delegate_itv)
{
    //TODO add itv to bidirectional search
    if constexpr (BiFmIndex<index_t>)
        search_algo_bi<abort_on_hit>(index, query, error_left, delegate);
    else
        search_algo_uni<abort_on_hit>(index, query, error_left, delegate, delegate_itv);
}
